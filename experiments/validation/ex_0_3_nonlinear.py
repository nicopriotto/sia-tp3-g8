"""Perceptron simple no lineal validado con y = tanh(x).

Run from repo root:
    python -m experiments.validation.ex_0_3_nonlinear --config experiments/validation/configs/nonlinear_clean.json
    python -m experiments.validation.ex_0_3_nonlinear --config experiments/validation/configs/nonlinear_noisy.json
"""
...
DEFAULT_CONFIG = "experiments/validation/configs/nonlinear_clean.json"
...
def plot_fit(
    ...
    x_line = np.linspace(X_train.min() - 0.1, X_train.max() + 0.1, 400).reshape(-1, 1)
    y_true_line = np.tanh(x_line)
    y_pred_line = model.predict(x_line)
    ax.plot(x_line, y_true_line, "g-", linewidth=1, alpha=0.7, label="y = tanh(x) (verdadera)")
    ax.plot(x_line, y_pred_line, "b--", linewidth=2, label="modelo aprendido")
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Perceptron simple no lineal - ajuste")
    ax.grid(alpha=0.3)
...
def main() -> None:
    ...
    print(f"Modelo aprendido: y = tanh({model.w[1]:.6f} x + {model.w[0]:.6f})")
