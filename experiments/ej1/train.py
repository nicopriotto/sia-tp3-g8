...
def main() -> None:
    ...
    model_init = build_model(config, n_features=bundle.X_train.shape[1])
    nets_initial = model_init._net(bundle.X_train).flatten()
    ...
    nets_final = model._net(bundle.X_train).flatten()
    ...
    for split_name, (X, y) in splits.items():
        ...
        evaluation[split_name] = {
            "mse": round(mse(y, preds), 6),
            "mae": round(mae(y, preds), 6),
            "n_samples": int(len(y)),
        }
    ...
    if not args.no_plots:
        ...
        plot_pred_vs_target(bundle.y_val, val_preds, out_dir / "pred_vs_target.png", title=f"Pred vs target (val) — {run_id}")
        plot_residuals_hist(bundle.y_val, val_preds, out_dir / "residuals_hist.png", title=f"Residuals (val) — {run_id}")
        plot_nets_histogram(nets_initial, nets_final, out_dir / "nets_saturation.png", title=f"Net distribution — {run_id}")
